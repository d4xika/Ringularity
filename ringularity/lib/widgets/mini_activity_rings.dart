...
class _MiniRingsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawRing(canvas, center, maxRadius, AppColors.accentBlue, stepsPercent, strokeWidth);

    // 2. Mittlerer Ring (Sleep - Cyan)
    _drawRing(canvas, center, maxRadius - strokeWidth - spacing, AppColors.accentCyan, sleepPercent, strokeWidth);

    // 3. Innerer Ring (Activity - Grün)
    _drawRing(canvas, center, maxRadius - (2 * (strokeWidth + spacing)), AppColors.accentGreen, activityPercent, strokeWidth);
  }

  void _drawRing(Canvas canvas, Offset center, double radius, Color color, double percent, double width) {
