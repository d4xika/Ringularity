...
/// A heavy data structure capturing all calculated averages, totals, and highly granular
/// trace points (graph coordinates) generated by the ring for a single specific calendar day.
class DailyVitals {
  final DateTime date;
  ...
  /// Creates a new [DailyVitals] block.
  DailyVitals({
    required this.date,
    required this.steps,
...
/// A short-term cache manager for the high-density graph data of the last 7 days.
///
/// Prevents the application from needing to hit the database or network every time
/// the user opens the app or swipes between recent days in the history views.
class VitalsStorageService extends ChangeNotifier {
  ...
  /// Creates a new [VitalsStorageService] and immediately loads local cache.
  ...
  /// Inserts or replaces a day's dataset, automatically discarding data older than 7 days.
  ...
  /// Keeps the local cache footprint minimal by dropping records older than 7 days.
  void _pruneOldData() {
  ...
  /// Synchronously fetches a full day's graph data from RAM if it exists.
